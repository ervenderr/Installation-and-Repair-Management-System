                                            ...
                                            <tbody>
                                                ...
                                                if(!isset($_GET['keyword'])){
                                                    ...
                                                    $query = "SELECT service_request.sreq_id, service_request.transaction_code, customer.fname, customer.lname, service_request.status, service_request.date_req
                                                        FROM service_request
                                                        JOIN customer ON service_request.Cust_id = customer.Cust_id
                                                        WHERE (service_request.transaction_code LIKE '%$keyword%' AND service_request.status = 'In-progress' OR service_request.status = 'Done'
                                                        OR customer.fname LIKE '%$keyword%' AND service_request.status = 'In-progress' OR service_request.status = 'Done'
                                                        OR customer.lname LIKE '%$keyword%' AND service_request.status = 'In-progress' OR service_request.status = 'Done'
                                                        )";
