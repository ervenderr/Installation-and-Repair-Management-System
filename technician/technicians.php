                            ...
                            <ul class="breadcrumb">
                                <li class="breadcrumb-item active btn-group-sm" aria-current="page">
                                    <button type="button" class="btn addnew" data-bs-toggle="modal"
                                        ...
                                        Create New technician <i class=" mdi mdi-plus "></i>
                                    </button>
                                </li>
                            </ul>
                        </nav>
                                            ...
                                            <tbody>
                                                ...
                                                if(!isset($_GET['keyword'])){
                                                    ...
                                                    while ($row = mysqli_fetch_assoc($result)) {
                                                        ...
                                                        echo '<td>';
                                                        echo '<a class="icns" href="view-technician.php?&rowid=' .  $row['tech_id'] . '">';
                                                        echo '<i class="fas fa-eye text-primary view-account" data-rowid="' .  $row['tech_id'] . '"></i>';
                                                        echo '</a>';
                                                    ...
                                                    // Perform the query
                                                    $query = "SELECT technician.tech_id, technician.fname, technician.lname, technician.phone, technician.address, technician.cust_type, accounts.email, accounts.user_type
                                                                FROM technician
                                                                JOIN accounts ON technician.account_id = accounts.account_id
                                                                WHERE (accounts.email LIKE '%$keyword%'AND technician.account_id = accounts.account_id
                                                                OR technician.fname LIKE '%$keyword%' AND technician.account_id = accounts.account_id
                                                                OR technician.lname LIKE '%$keyword%' AND technician.account_id = accounts.account_id
                                                                )";
                                                    ...
                                                    while ($row = mysqli_fetch_assoc($result)) {
                                                        ...
                                                        echo '<td>' . $row['address'] . '</td>';
                                                        echo '<td>';
                                                        echo '<a class="icns" href="view-technician.php?&rowid=' . $id . '">';
                                                        echo '<i class="fas fa-eye text-primary view-account" data-rowid="' . $id . '"></i>';
                                                        echo '</a>';
                                                        echo '<a class="icns" href="edit-technician.php?&rowid=' . $id . '">';
                                                        echo '<i class="fas fa-edit text-success view-account" data-rowid="' . $id . '"></i>';
                                                        echo '</a>';
                                                        echo '<a class="icns" href="delete-technician.php?&rowid=' . $id . '">';
