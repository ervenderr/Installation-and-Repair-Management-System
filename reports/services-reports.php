                ...
                <div class="content-wrapper report-content">
                    <div class="page-header report-header">
                        <ul class="nav nav-pills nav-justified report-nav">
                            ...
                            <li class="nav-item">
                                <a class="nav-link active" href="services-reports.php">Services Sales Report</a>
                            </li>
                            <li class="nav-item">
                    ...
                    <?php
                        // Total sales today
                        $sql_today = "SELECT SUM(payment) as total FROM service_request WHERE DATE(date_completed) = CURDATE() AND status='Completed'";
                        $result_today = $conn->query($sql_today);
                        $salesToday = $result_today->fetch_assoc()['total'] ?? 0;

                        // Total sales this month
                        $sql_month = "SELECT SUM(payment) as total FROM service_request WHERE YEAR(date_completed) = YEAR(CURRENT_DATE()) AND MONTH(date_completed) = MONTH(CURRENT_DATE()) AND status='Completed'";
                        $result_month = $conn->query($sql_month);
                        $salesThisMonth = $result_month->fetch_assoc()['total'] ?? 0;

                        // Total sales this year
                        $sql_year = "SELECT SUM(payment) as total FROM service_request WHERE YEAR(date_completed) = YEAR(CURRENT_DATE()) AND status='Completed'";
                        $result_year = $conn->query($sql_year);
                        $salesThisYear = $result_year->fetch_assoc()['total'] ?? 0;

                        // Total completed request
                        $sql_completed = "SELECT COUNT(*) as total FROM service_request WHERE status='Completed'"; // Adjust this query based on your criteria for a completed request
                        ...
                        <div class="card-body">
                            <div class="row mg-btm">
                                <div class="col-sm-12 col-md-6 flex text-center">
                                    <h4 class="card-title">Service Sales Report</h4>
                                </div>
                            </div>
                            <div class="row">
                                ...
                                <div class="col-12 grid-margin">
                                    <div class="table-responsive text-center">
                                        ...
                                        <table id="myDataTable" class="table table-hover">
                                            ...
                                            $sql = "SELECT EXTRACT(YEAR FROM date_completed) as year, EXTRACT(MONTH FROM date_completed) as month, SUM(payment) as total FROM service_request WHERE status = 'Completed' GROUP BY EXTRACT(YEAR FROM date_completed), EXTRACT(MONTH FROM date_completed) ORDER BY EXTRACT(YEAR FROM date_completed), EXTRACT(MONTH FROM date_completed)";
                    ...
                    <div class="card report-card-two text-center">
                        <div class="card-body">
                            <h4 class="card-title">Service Transaction Record</h4>
                            <div class="row">
                                <div class="col-12 grid-margin">
                                    <div class="table-responsive">
                                        ...
                                        <table class="table table-hover" id="myDataTable2">
                                            ...
                                            <tbody id="myTable2">
                                                <?php
                                            $sql_all = "SELECT * FROM service_request WHERE status='Completed' ORDER BY date_completed DESC";
